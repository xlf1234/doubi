...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/xlf1234/doubi/master/bbr.sh; then
...
Download_SSR(){
	cd "/usr/local"
	 #wget -N --no-check-certificate "https://github.com/xlf1234/shadowsocksr/archive/manyuser.zip"
	wget -N --no-check-certificate "https://github.com/xlf1234/doubi/blob/master/manyuser.zip"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xlf1234/doubi/master/other/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xlf1234/doubi/master/other/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
JQ_install(){
	if [[ ! -e ${jq_file} ]]; then
		...
		if [[ ${bit} = "x86_64" ]]; then
			...
			wget --no-check-certificate "https://github.com/xlf1234/doubi/blob/master/jq-linux64" -O ${jq_file}
		else
			#mv "jq-linux32" "jq"
			#wget --no-check-certificate "https://github.com/stedolan/jq/releases/download/jq-1.5/jq-linux32" -O ${jq_file}
			wget --no-check-certificate "https://github.com/xlf1234/doubi/blob/master/jq-linux32" -O ${jq_file}
		fi
...
Install_LotServer(){
	...
	wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/xlf1234/serverSpeeder_Install/master/appex.sh"
...
Uninstall_LotServer(){
	...
	[[ -z ${unyn} ]] && echo && echo "已取消..." && exit 1
	if [[ ${unyn} == [Yy] ]]; then
		wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/xlf1234/serverSpeeder_Install/master/appex.sh" && bash /tmp/appex.sh 'uninstall'
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/xlf1234/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/xlf1234/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	...
	if [[ -z ${softs_domain} ]]; then
		softs_domain="https://raw.githubusercontent.com/xlf1234/doubi/master/"
	else
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/xlf1234/doubi/master/ssrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ $sh_new_type == "softs" ]]; then
		...
		wget -N --no-check-certificate "https://raw.githubusercontent.com/xlf1234/doubi/master/ssrmu.sh" && chmod +x ssrmu.sh
	fi
